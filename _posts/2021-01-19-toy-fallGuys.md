...
폴 가이 토이 프로젝트를 진행하게 된 이유는 두 가지였다. 하나는 예전부터 "개인적으로 정말 만들고 싶은 것"을 내 손으로 만드려는 욕구를 해소하고 싶었기 때문이고, 다른 하나는 Interactive Developer님의 이야기가 크게 와닿았기 때문이다. 어도비 플래시에 관한 내용으로 개발 툴의 패러다임이 바뀌어도 적응할 수 있는 기초를 다져야 한다는 내용이었다.   
